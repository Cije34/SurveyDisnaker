                ...
                @forelse ($pesertas as $peserta)
                    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-md" x-data="{ open: false }">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="font-semibold text-slate-800">{{ $peserta->user->name ?? '-' }}</h3>
                                <p class="text-sm text-slate-500">{{ $peserta->nik ?? '-' }}</p>
                            </div>
                        ...
                        <div class="mt-4 space-y-2 border-t border-slate-100 pt-4 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-slate-700">{{ $peserta->pendidikan_terakhir ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Email:</span>
                                <span class="font-medium text-slate-700">{{ $peserta->email ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">No. Telepon:</span>
                                <span class="font-medium text-slate-700">{{ $peserta->no_hp ?? '-' }}</span>
                            </div>
                        </div>
                    </div>
                @empty
