<x-admin.layout :user="$user" active="penjab" title="Penanggung Jawab">
    ...
    <div class="px-6 py-8 lg:px-10 lg:py-10 space-y-8" x-data="penjabPage()">
        @if (session('status'))
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_30px_55px_-35px_rgba(15,23,42,0.35)]">
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead>
                        <tr class="bg-sky-700">
                            ...
                            <th scope="col" class="px-6 py-4 text-left text-sm font-semibold uppercase tracking-wide text-white">Jabatan</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($penjabs as $penjab)
                             <tr class="hover:bg-slate-50" x-data="{ open: false }" :class="{ 'relative z-10': open }">
                                 <td class="whitespace-nowrap px-6 py-4 text-sm font-semibold text-slate-800">{{ $penjab->name ?? '-' }}</td>
                                 <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{{ $penjab->jabatan ?? '-' }}</td>
                                 <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{{ $penjab->email ?? '-' }}</td>
                                 <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{{ $penjab->no_hp ?? '-' }}</td>
                                 <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{{ $penjab->jenis_kelamin ?? '-' }}</td>
                                     ...
                                     <div class="relative inline-flex">
                                         ...
                                         <div x-show="open"
                                             ...
                                             <button type="button"
                                                     ...
                                                     data-id="{{ $penjab->id }}"
                                                     data-name="{{ $penjab->name }}"
                                                     data-jabatan="{{ $penjab->jabatan }}"
                                                     data-email="{{ $penjab->email }}"
                                                     data-no_hp="{{ $penjab->no_hp }}"
                                                     data-jenis_kelamin="{{ $penjab->jenis_kelamin }}"
                                                     data-alamat="{{ $penjab->alamat }}"
                ...
                @forelse ($penjabs as $penjab)
                    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-md" x-data="{ open: false }">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="font-semibold text-slate-800">{{ $penjab->name ?? '-' }}</h3>
                                <p class="text-sm text-slate-500">{{ $penjab->jabatan ?? '-' }}</p>
                            </div>
                            <div class="relative flex-shrink-0">
                                <button type="button"
                                 ...
                                 <div x-show="open" x-transition.origin.top.right x-cloak class="absolute right-0 top-full z-10 mt-2 w-40 rounded-xl border bg-white py-2 shadow-lg">
                                     <button type="button" @click="open = false; openEditModal($event.target.dataset)" data-id="{{ $penjab->id }}" data-name="{{ $penjab->name }}" data-jabatan="{{ $penjab->jabatan }}" data-email="{{ $penjab->email }}" data-no_hp="{{ $penjab->no_hp }}" data-jenis_kelamin="{{ $penjab->jenis_kelamin }}" data-alamat="{{ $penjab->alamat }}" class="flex w-full items-center gap-2 px-4 py-2 text-left text-sm text-slate-600 hover:bg-slate-100">Edit</button>
                                     <button type="button" @click="open = false; confirmDelete($event.target.dataset)" data-id="{{ $penjab->id }}" data-nama="{{ $penjab->name }}" class="flex w-full items-center gap-2 px-4 py-2 text-left text-sm text-rose-500 hover:bg-rose-50">Hapus</button>
                                 </div>
                            </div>
                        </div>
                        <div class="mt-4 space-y-2 border-t border-slate-100 pt-4 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-slate-700">{{ $penjab->email ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">No. Telepon:</span>
                                <span class="font-medium text-slate-700">{{ $penjab->no_hp ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Jenis Kelamin:</span>
                                <span class="font-medium text-slate-700">{{ $penjab->jenis_kelamin ?? '-' }}</span>
                            </div>
                        </div>
                    </div>
                @empty
             ...
             <div x-show="importOpen"
                 ...
                 <div class="rounded-2xl sm:rounded-3xl bg-sky-900/95 max-h-[85vh] overflow-y-auto">
                     <div class="p-4 sm:p-6">
                         ...
                         <div class="mt-5">
                             <a href="{{ route('admin.penjab.template') }}"
                     ...
                     <div class="px-4 sm:px-6 pb-4 sm:pb-6">
                         <div class="rounded-2xl sm:rounded-3xl bg-sky-800/70 p-6 sm:p-8 text-center text-slate-100">
                             ...
                             <form method="POST" action="{{ route('admin.penjab.import') }}" enctype="multipart/form-data" class="mt-6">
                                 @csrf
             ...
             <div x-show="modalOpen"
                 ...
                 <div class="rounded-3xl bg-white/95 p-6">
                     <div class="mb-4 flex items-center justify-between">
                         <h3 class="text-lg font-semibold text-slate-900">Tambah Penanggung Jawab Baru</h3>
                     ...
                     <form method="POST" action="{{ route('admin.penjab.store') }}" class="space-y-4 pt-2">
                         @csrf
                         <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                             ...
                             <div>
                                 <label for="jabatan" class="block text-xs font-semibold uppercase tracking-wide text-slate-500">Jabatan</label>
         ...
         <div x-show="editOpen"
             ...
             <div x-show="editOpen"
                 ...
                 <div class="rounded-3xl bg-white/95 p-6">
                     <div class="mb-4 flex items-center justify-between">
                         <h3 class="text-lg font-semibold text-slate-900">Edit Penanggung Jawab</h3>
                     ...
                     <form method="POST" :action="`/admin/penjab/${editMeta?.id}`" class="space-y-4 pt-2">
                         @csrf
                         @method('PUT')
                         <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                             <div>
                             ...
                             <div>
                                 <label for="edit_jabatan" class="block text-xs font-semibold uppercase tracking-wide text-slate-500">Jabatan</label>
                                 <input id="edit_jabatan" type="text" name="jabatan" :value="editMeta?.jabatan" required
                                        class="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-5 py-2.5 text-sm text-slate-800 focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-100" />
                             </div>
                         </div>
                         <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
         ...
         <div x-show="deleteOpen"
             ...
             <div x-show="deleteOpen"
                 ...
                 <form method="POST" :action="deleteAction()" class="rounded-3xl bg-white/95 p-4 sm:p-6 space-y-4 sm:space-y-5">
                     ...
                     <p>
                         Apakah Anda yakin ingin menghapus penanggung jawab <strong x-text="editMeta?.nama" class="font-semibold text-slate-800"></strong>?
...
<script>
    function penjabPage() {
        return {
            ...
            deleteOpen: false,
            editMeta: null,
            deleteTemplate: '{{ route('admin.penjab.destroy', '__ID__') }}',
            openEditModal(data) {
